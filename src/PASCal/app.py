...
def output():
    ...
    Latt = np.stack(Latt)
    Vol = PASCal.utils.cell_vol(Latt)  # volumes in (Angstrom^3)
    ## Data error handling
    if len(TPx) < 2:
    ...
    if DataType == "Pressure":
        if len(TPx) < 4:
            warning.append(
                "At least as many data points as parameters are needed for a fit to be carried out (e.g. 3 for 3rd order Birch-Murnaghan, 4 for empirical pressure fitting). As PASCal.utils calculates errors from derivatives, more data points than parameters are needed for error estimates."
            )
        if UsePc == "True":
    ...
    if DataType == "Electrochemical":
        if len(TPx) - 2 < DegPolyCap:
            ...
            warning.append(
                "The maximum degree of the Chebyshev strain polynomial has been lowered. At least as many data points as parameters are needed for a fit to be carried out. As PASCal.utils calculates errors from derivatives, more data points than parameters are needed for error estimates."
            )
        if len(TPx) - 2 < DegPolyVol:
            DegPolyVol = len(TPx) - 2
            warning.append(
                "The maximum degree of the Chebyshev volume polynomial has been lowered. At least as many data points as parameters are needed for a fit to be carried out. As PASCal.utils calculates errors from derivatives, more data points than parameters are needed for error estimates."
            )
    ...
    ### Strain calculations
    Orth = PASCal.utils.orthomat(Latt[:])  # cell in orthogonal axes
    ...
    if DataType == "Pressure":
        ...
        for i in range(3):
            ...
            K[i][:] = (
                PASCal.utils.Comp(TPx[:], CalEmPopt[i][1], CalEmPopt[i][2], CalEmPopt[i][3])
                * TPa
            )  # compressibilities (TPa^-1) so multiply by 1e3

            KErr[i][:] = (
                PASCal.utils.CompErr(
                    CalEmPcov[i],
                    TPx[:],
                    CalEmPopt[i][1],
                    CalEmPopt[i][2],
        ...
        if UsePc == "True":  ## if a critical pressure is USED
            PoptThirdBMPc, PcovThirdBMPc = curve_fit(
                PASCal.utils.WrapperThirdBMPc(PcVal),
                Vol,
                TPx,
        ...
        SigBPrime = [
            "n/a",
            PASCal.utils.round_array(SigBprimeThirdBM, 4),
        ...
        if UsePc == "True":  ## add in critical pressure values
            ...
            SigBPrime.append(PASCal.utils.round_array(SigBprimeThirdBMPc, 4))
            PcCoef = np.concatenate([PcCoef, [PcVal]])

        ### Compute the pressure from all fits
        CalPress = np.zeros((3, Latt.shape[0]))
        CalPress[0][:] = (Vol - VolYInt) / VolGrad  # not the same as PASCal.utils?
        CalPress[1][:] = PASCal.utils.SecBM(Vol[:], PoptSecBM[0], PoptSecBM[1])
        CalPress[2][:] = PASCal.utils.ThirdBM(
            Vol[:], PoptThirdBM[0], PoptThirdBM[1], PoptThirdBM[2]
        )
        if UsePc == "True":  ## if a critical pressure is USED
            PThirdBMPc = PASCal.utils.ThirdBMPc(
        ...
        FigVolume.add_trace(
            go.Scatter(
                ...
                x=PASCal.utils.SecBM(np.linspace(Vol[0], Vol[-1], num=100), *PoptSecBM),
        ...
        FigVolume.add_trace(
            go.Scatter(
                ...
                x=PASCal.utils.ThirdBM(np.linspace(Vol[0], Vol[-1], num=100), *PoptThirdBM),
                y=np.linspace(Vol[0], Vol[-1], num=100),
                mode="lines",
    ...
    ####
    ## Indicatrix 3D plot
    NormCrax = PASCal.utils.NormCRAX(CrystPrinAx[u, :, :], PrinComp)
    maxIn, R, X, Y, Z = PASCal.utils.Indicatrix(PrinComp)
    ...
    if DataType == "Temperature":
        return render_template(
            ...
            data=data,
            Axes=Axes,
            PrinComp=PASCal.utils.round_array(PrinComp, 4),
            CalAlphaErr=PASCal.utils.round_array(CalAlphaErr * MK, 4),
            MedianPrinAxCryst=PASCal.utils.round_array(MedianPrinAxCryst, 4),
            PrinAxCryst=PASCal.utils.round_array(PrinAxCryst, 4),
            TPx=TPx,
            DiagStrain=PASCal.utils.round_array(DiagStrain * percent, 4),
            XCal=PASCal.utils.round_array(XCal, 4),
            Vol=PASCal.utils.round_array(Vol, 4),
            VolLin=PASCal.utils.round_array(VolLin, 4),
            VolCoef=PASCal.utils.round_array(VolCoef, 4),
            VolCoefErr=PASCal.utils.round_array(VolCoefErr, 4),
            TPxError=TPxError,
            Latt=Latt,
        )
    ...
    if DataType == "Pressure":
        return render_template(
            ...
            CalEpsilon0=PASCal.utils.round_array(CalEpsilon0, 4),
            CalLambda=PASCal.utils.round_array(CalLambda, 4),
            CalPc=PASCal.utils.round_array(CalPc, 4),
            CalNu=PASCal.utils.round_array(CalNu, 4),
            StrainHeadings=StrainHeadings,
            InputHeadings=InputHeadings,
            data=data,
            Axes=Axes,
            PrinComp=PASCal.utils.round_array(PrinComp, 4),
            KErr=PASCal.utils.round_array(KErr, 4),
            u=u,
            MedianPrinAxCryst=PASCal.utils.round_array(MedianPrinAxCryst, 4),
            PrinAxCryst=PASCal.utils.round_array(PrinAxCryst, 4),
            BMCoeffHeadings=BMCoeffHeadings,
            BMOrder=BMOrder,
            B0=PASCal.utils.round_array(B0, 4),
            SigB0=PASCal.utils.round_array(SigB0, 4),
            V0=PASCal.utils.round_array(V0, 4),
            SigV0=PASCal.utils.round_array(SigV0, 4),
            BPrime=PASCal.utils.round_array(BPrime, 4),
            SigBPrime=SigBPrime,
            PcCoef=PASCal.utils.round_array(PcCoef, 4),
            KHeadings=KHeadings,
            K=PASCal.utils.round_array(K, 4),
            TPx=TPx,
            DiagStrain=PASCal.utils.round_array(DiagStrain * percent, 4),
            XCal=PASCal.utils.round_array(XCal, 4),
            ...
            VolCoef=PASCal.utils.round_array(VolCoef, 4),
            VolCoefErr=PASCal.utils.round_array(VolCoefErr, 4),
            CalPress=PASCal.utils.round_array(CalPress, 4),
            UsePc=UsePc,
            TPxError=TPxError,
            Latt=Latt,
        )
    ...
    if DataType == "Electrochemical":
        return render_template(
            ...
            Vol=PASCal.utils.round_array(Vol, 4),
            Deriv=PASCal.utils.round_array(Deriv, 4),
            VolElecHeadings=VolElecHeadings,
            VolCheb=PASCal.utils.round_array(VolCheb, 4),
            VolCoef=PASCal.utils.round_array(VolCoef, 4),
            InputHeadings=InputHeadings,
            TPxError=TPxError,
            Latt=PASCal.utils.round_array(Latt, 4),
        )
