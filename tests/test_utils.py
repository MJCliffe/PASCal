from PASCal.utils import round_array, orthomat, cell_vol, Round, Orthomat, CellVol
import numpy as np
import random
...
def test_round_array():
    ...
    while tests < 100:
        ...
        for _I in [I1, I2, I3]:
            ...
            assert Round(_I * c, 1).all() == round_array(_I * c, 1).all()
...
def test_orthomat():
    ...
    assert orth.all() == Orthomat(np.hstack((lattice, lattice))).all()
...
def test_cell_vol():
    ...
    assert cell_vol(np.hstack((lattice, lattice2))).all() == CellVol(np.hstack((lattice, lattice2))).all()
